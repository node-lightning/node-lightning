        ...
        describe("when tx cant be found in block", () => {
            it("should abort processing and return error", async () => {
                ...
                chainClient.getBlock.resolves({
                    ...
                    nTx: 76,
                    previousblockhash:
                        "00000000a35543bec4e7f228e07bdd7a624e442237d26fac28269c645ec52df2",
                    nextblockhash:
                        "000000001350700fbcb7864b703df721247ff959e3e56f574a49db761b20b35f",
                });
                const rawMsgs = [
                    "",
                ];
            ...
            it("should abort processing and return error", async () => {
                ...
                chainClient.getUtxo.resolves({
                    ...
                    scriptPubKey: {
                        ...
                        addresses: [
                            "tb1qlhhjkgdcy72emjhn6v0c7ru9nnvpa3wnx4s5a48r8rxl3n5adl9syhd965",
                        ],
                    },
                    coinbase: false,
                });
    ...
    describe("message permutations", () => {
        ...
        for (let i = 0; i < permutations.length; i++) {
            it("graph match on permutation " + (i + 1), async () => {
                ...
                expect(gossipStore.nodeAnnouncementCount).to.deep.equal(
                    refStore.nodeAnnouncementCount,
                );
                expect(pendingStore.channelUpdateCount).to.equal(0);
                expect(pendingStore.nodeAnnouncementCount).to.equal(0);
            });
