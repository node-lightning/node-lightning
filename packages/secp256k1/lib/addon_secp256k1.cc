...
#define PUBKEY_SERIALIZE(retcode)                                              \
  do {                                                                         \
    ...
    RETURN_IF_ZERO(secp256k1_ec_pubkey_serialize(this->ctx_, output.Data(),    \
                                                 &outputlen, &pubkey, flags),  \
                   retcode);                                                   \
  } while (0)
...
Napi::Value Secp256k1Addon::PublicKeyVerify(const Napi::CallbackInfo& info) {
  ...
  RETURN_IF_ZERO(secp256k1_ec_pubkey_parse(this->ctx_, &pubkey, input.Data(),
                                           input.Length()),
                 1);
  RETURN(0);
}
...
Napi::Value Secp256k1Addon::PublicKeyCombine(const Napi::CallbackInfo& info) {
  ...
  for (unsigned int i = 0; i < inputs.Length(); ++i) {
    ...
    RETURN_IF_ZERO(secp256k1_ec_pubkey_parse(this->ctx_, &pubkeys[i],
  ...
  secp256k1_pubkey pubkey;
  RETURN_IF_ZERO(secp256k1_ec_pubkey_combine(this->ctx_, &pubkey, ptrs.get(),
                                             inputs.Length()),
                 2);
  PUBKEY_SERIALIZE(3);
  RETURN(0);
...
Napi::Value Secp256k1Addon::SignatureExport(const Napi::CallbackInfo& info) {
  ...
  RETURN_IF_ZERO(secp256k1_ecdsa_signature_serialize_der(this->ctx_, output,
                                                         &outputlen, &sig),
                 2);
...
int ecdsa_sign_nonce_function(unsigned char* nonce32,
  ...
  auto result = obj->fn.Call({obj->msg32, obj->key32, env.Null(), obj->data,
...
Napi::Value Secp256k1Addon::ECDSASign(const Napi::CallbackInfo& info) {
  ...
  RETURN_IF_ZERO(secp256k1_ecdsa_sign_recoverable(this->ctx_, &sig, msg32,
                                                  seckey, noncefn, data),
                 1);
...
// ECDH
int ecdh_hash_function(unsigned char* output, const unsigned char* x,
