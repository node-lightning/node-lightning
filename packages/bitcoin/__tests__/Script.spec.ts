...
describe("Script", () => {
    ...
    describe("#parse()", () => {
        ...
        it("op_pushdata1", () => {
            const buf = Buffer.from("4c64" + "00".repeat(100), "hex");
            const script = Script.fromBuffer(buf);
            expect((script.cmds[0] as Buffer).toString("hex")).to.equal("00".repeat(100));
        });

        it("op_pushdata2", () => {
            const buf = Buffer.from("4d0001" + "00".repeat(256), "hex");
            const script = Script.fromBuffer(buf);
    ...
    describe(".serialize()", () => {
        ...
        it("op_code", () => {
            ...
            expect(script.serializeCmds().toString("hex")).to.equal("76");
        });

        it("op_rawpushbytes", () => {
            const script = new Script();
            script.cmds.push(Buffer.alloc(1));
            expect(script.serializeCmds().toString("hex")).to.equal("0100");
        });

        it("op_pushdata1", () => {
            const script = new Script();
            script.cmds.push(Buffer.alloc(100));
            expect(script.serializeCmds().toString("hex")).to.equal("4c64" + "00".repeat(100));
        });

        it("op_pushdata2", () => {
            const script = new Script();
            script.cmds.push(Buffer.alloc(256));
            expect(script.serializeCmds().toString("hex")).to.equal("4d0001" + "00".repeat(256));
        });
    });
