        ...
        vout: [
          ...
          },
          {
            ...
            scriptPubKey: {
              ...
              addresses: ['bc1qwqdg6squsna38e46795at95yu9atm8azzmyvckulcc7kytlcckxswvvzej'],
            },
          },
        ],
      },
  ...
  for (let fixture of fixtures) {
    it(fixture.assertion, () => {
      ...
      for (let i = 0; i < result.vin.length; i++) {
        ...
        if (expected.vin[i].txinwitness) {
          ...
          for (let w = 0; w < vin.witness.length; w++) {
            expect(vin.witness[w].toString('hex')).to.equal(expected.vin[i].txinwitness[w]);
          }
        }
      }
      ...
      for (let i = 0; i < result.vout.length; i++) {
        ...
        expect(vout.value.toNumber()).to.equal(expected.vout[i].value * 10 ** 8);
        expect(vout.pubKeyScript.toString('hex')).to.equal(expected.vout[i].scriptPubKey.hex);
      }
