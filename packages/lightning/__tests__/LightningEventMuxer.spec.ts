    ...
    describe(LightningEventMuxer.prototype.onEvent.name, () => {
        ...
        describe("peer_ready", () => {
            it("calls GossipManager", async () => {
                ...
                // assert
                expect(result.isOk).to.equal(true);
                expect(gossipManager.onPeerReady.called).to.equal(true);
                expect(gossipManager.onPeerReady.args[0][0]).to.equal(peer);
            });
        });
        ...
        describe("peer_message", () => {
            ...
            it(`routes ${MessageType.FundingCreated} - ${MessageType[MessageType.FundingCreated]}`);
